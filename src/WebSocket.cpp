...
WebSocket::WebSocket(char* network, char* password, char* server, int portId,
                                                                String botId)
{
    Serial.print("in websocket initialization");
    ssid = network;
    pass = password;
    port = portId;
    ...
    WiFiClient wifi;
    client = WebSocketClient(wifi, serverAddress, port);
    clientID = "56FC703ACE1A"; //Insert your Client ID Here!
    int status = WL_IDLE_STATUS;
...
void WebSocket::loop() {
    int count = 0;
    Serial.println("In websocket loop, starting WebSocket connection...");
